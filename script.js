...
const mobileClose = document.getElementById('mpClose');
const mobileVolume = document.getElementById('mpVolume');
let mobileOpen = false;

function setMobilePlayerOpen(open) {
  mobileOpen = open;
  mobilePlayer.hidden = !open;
  mobilePlayer.classList.toggle('open', open);

  pipButtons.forEach(button => button.classList.toggle('active', open));
}
...
mobileClose.addEventListener('click', closeMobilePlayer);

if (mobileVolume) {
  mobileVolume.value = String(radio.volume);

  mobileVolume.addEventListener('input', () => {
    const value = parseFloat(mobileVolume.value);
